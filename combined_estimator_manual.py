...
class CombinedPoseEstimator:
    def __init__(self, yolo_model_path, camera_index=1):
        ...
        self.orb = cv2.ORB_create(
            nfeatures=3000,        # Even more features (was 3000)
            scaleFactor=1.2,
            nlevels=8,
            edgeThreshold=25,
        ...
        print(f"✓ ORB initialized (3000 features, sensitive mode)")
        ...
        # Object waiting to be calibrated
        self.pending_calibration_object = None
    ...
    def run(self):
        ...
        print("="*60)
        print("\nControls:")
        print("  'c' - Calibrate prompted object")
        ...
        try:
            while True:
                ...
                for obj_name, state in self.targets.items():
                    ...
                    # Calibration logic (homography only)
                    if state["method"] == "homography" and not state["calibrated"]:
                        if bbox is not None and conf > YOLO_CONFIDENCE_THRESHOLD:
                            ...
                            if is_stationary and state["ready_to_calibrate"]:
                                self.pending_calibration_object = obj_name
                            ...
                            if self.pending_calibration_object == obj_name:
                                self.pending_calibration_object = None
                ...
                # UI
                if self.pending_calibration_object is not None:
                    obj_name = self.pending_calibration_object
                    prompt_text = f"{obj_name.upper()} ready. Press 'c' to CALIBRATE."
                    
                    text_size = cv2.getTextSize(prompt_text, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)[0]
                    ...
                    cv2.putText(display_frame, prompt_text, (text_x, text_y),
                ...
                if key == 27:
                    ...
                    if self.pending_calibration_object is not None:
                        ...
                        if bbox is not None and state["method"] == "homography":
                            ...
                            print(f"Cannot calibrate {obj_name}")
                            self.pending_calibration_object = None
                    else:
                        print("No object ready for calibration")
