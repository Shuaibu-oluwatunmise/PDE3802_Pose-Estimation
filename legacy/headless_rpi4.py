...
class Full5ObjectTracker(Node):
    ...
    def draw_axes(self, display, obj_name, rvec, tvec):
        pass # No visual output in headless mode

    def run(self):
        if not self.camera.start(): return
        self.get_logger().info("Setup Complete. Starting HEADLESS Async Loop...")
        # print("FPS Logging Only - No GUI")
            ...
            while rclpy.ok():
                ...
                for name, specs in OBJECT_SPECS.items():
                    ...
                    # --- FEATURE METHOD ---
                        ...
                        if is_fresh_yolo:
                            bbox = current_yolo_detections[name]
                        elif tracker.is_tracking:
                            bbox = tracker.predict_next_bbox(frame.shape) 
