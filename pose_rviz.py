"""
Complete Pose Estimation System with Reference Loading - RASPBERRY PI + ROS2 JAZZY
...
- 🤖 ROS2 JAZZY TF PUBLISHING:
  - Publishes TF frames for all 5 objects
  - Parent frame: camera_link
  - Child frames: {object}_frame
"""
...
class ReferencePoseEstimator(Node):  # 🤖 Inherit from ROS2 Node
    def __init__(self, yolo_model_path, reference_dir="reference_images"):
        # 🤖 ROS2: Initialize node FIRST
        ...
        # 🚀 PI OPTIMIZATION: Lighter ORB (25% less computation)
    ...
    def publish_tf(self, obj_name, rvec, tvec):
        """Publish TF frame for object to ROS2"""
        t = TransformStamped()
    ...
    def run(self):
        """Main loop"""
        print("\n" + "="*60)
        print("REFERENCE-BASED POSE ESTIMATION - RPI + ROS2 JAZZY")
        print("="*60)
        print("\nFeatures:")
        ...
        print("  • 🤖 ROS2 TF publishing to RViz")
        print("\nControls:")
        print("  's' - Save frame")
        print("  ESC - Exit")
        print("="*60 + "\n")
        ...
        try:
            while True:
                ...
                if tracked_count > 0:
                    cv2.putText(display_frame, f"TRACKING {tracked_count}/5 + TF", 
                                (10, display_frame.shape[0]-20),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                
                cv2.imshow('RPI Pose Estimation + ROS2', display_frame)
                ...
                if key == 27:
                    ...
                    filename = f"rpi_ros2_tracking_{saved_count:03d}.jpg"
...
def main():
    print("\n" + "="*60)
    print("SETUP - RASPBERRY PI + ROS2 JAZZY VERSION")
    print("="*60)
